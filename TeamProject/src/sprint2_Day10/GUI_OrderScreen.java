 ...
 */
      ...
      //create JTable for bottom Panel load test data for gui design   
        DefaultTableModel dtm = new DefaultTableModel()
        {
         ...
         JLabel orderslabel = new JLabel("All Orders"); //Create table title
         SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
         JLabel dateLabel = new JLabel("System Date: "+ sdf.format(NewUI.systemDate)); //Date String
         
         //Create Advance Date button and set action listener
         JButton advanceDateButton = new JButton("Advance Date"); 
         advanceDateButton.setActionCommand(homeScreenAccess);   
         advanceDateButton.addActionListener(new ActionListener(){ 
            @Override
         ...
         JPanel tableHeaderJP = new JPanel();
         tableHeaderJP.setLayout(new BoxLayout(tableHeaderJP,BoxLayout.X_AXIS));   
         tableHeaderJP.add(orderslabel);
         tableHeaderJP.add(Box.createRigidArea(new Dimension(280,0)));
         tableHeaderJP.add(dateLabel);
         tableHeaderJP.add(Box.createRigidArea(new Dimension(20,0)));
         tableHeaderJP.add(advanceDateButton);
        ...
        if (NewUI.currentUser.isAdmin()){
         ...
         botJP.add(tableHeaderJP, BorderLayout.NORTH);  
