...
//        screen.setLayout(new BoxLayout(customerViewScreen,BoxLayout.Y_AXIS))  ;     
        ...
        editButton.addActionListener(new ActionListener() { 
      	  public void actionPerformed(ActionEvent e){
      			  for (Customer c : NewUI.db.getCustomers()){
      				  if (NewUI.selectedCustomerID == c.getId()){
      					  if (NewUI.check.isNotBlank(nameText.getText()) && NewUI.check.isNotBlank(addressText.getText())
        ...
        botJP.add(middleDetailsJP,BorderLayout.CENTER);
        ...
        middleInvoicesJP.add(new JLabel("Total money spent: " + customer.getTotalMoneySpent(NewUI.db.getInvoices())), BorderLayout.SOUTH);
        
        botJP.add(middleInvoicesJP,BorderLayout.SOUTH);
      	    	  ...
      	    	  if (NewUI.selectedCustomerID==c.getId()){
      	    		  ...
      	    		  for(Invoice invoice : NewUI.db.getInvoices()){
                		  if(NewUI.selectedCustomerID == invoice.getCustomer().getId()){
                			  String oldName = invoice.getCustomer().getName();
                			  invoice.getCustomer().setName(oldName+" <Deleted>");
                			  break;
                		  }
                	  }      	                         
      	    		  NewUI.selectedCustomerID=0;
