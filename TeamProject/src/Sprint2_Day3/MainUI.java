...
public class MainUI extends JFrame {
	...
	private JMenu filemenu, productsmenu, ordersmenu, custumersmenu,
			suppliersmenu, invoicesmenu, stockControlmenu;
	// JMenuItem variable for menu bar on screen
	private JMenuItem homeView, eExit, productsView, ordersView, customerView,
			supplierView, invoicesView, reportsView, addRandomDatabase;
	// Stings identify the JPanels screens in the CardLayout
	private final String homeScreenAccess = "home",
			productScreenAccess = "product", orderScreenAccess = "order",
			supplierScreenAccess = "supplierEdit",
			customerScreenAccess = "customer", invoiceScreenAccess = "invoice",
			productCreateScreenAccess = "productCreate",
			productEditScreenAccess = "productEdit",
			orderCreateScreenAccess = "orderCreate",
			orderEditScreenAccess = "orderEdit",
			supplierCreateScreenAccess = "supplierCreate",
			customerCreateScreenAccess = "customerCreate",
			customerViewScreenAccess = "customerView",
			invoiceCreateScreenAccess = "invoiceCreate",
			invoiceEditScreenAccess = "invoiceEdit",
	...
	private JPanel contentPane, gui, homeScreen, productScreen, orderScreen,
			supplierScreen, customerScreen, invoiceScreen, productCreateScreen,
			productEditScreen, orderCreateScreen, orderEditScreen,
			supplierCreateScreen, customerCreateScreen, customerViewScreen,
	...
	private JComboBox<String> createOrderSupplierCombo,
			createOrderProductCombo, createInvoiceCustomerCombo,
			createInvoiceProductCombo;
	...
	Vector<String> listOfOrdersVector, listOfOrderProductsVector,
			listOfInvoiceProductsVector, listOfSuppliersVector,
			listOfInvoicesVector, listOfCustomersVector;
	// Integer variables used for combo box position indexes
	int createOrderSupplierIndex, createInvoiceCustomerIndex,
			createInvoiceSupplierIndex, selectedCustomerID;
	//
	JMenuBar menubar;
	// String ArrayList to store the Products in basket to be added to an
	// Order/Invoices
	private ArrayList<Product> orderBasket = new ArrayList<Product>(),
			invoiceBasket = new ArrayList<Product>();
	...
	public void homeScreen() {
		...
		JLabel titlelbl = new JLabel("Retail Management System Home",
				JLabel.CENTER);
	...
	public void stockControlScreen() {
		...
		viewChartButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				stockControlBotJP.add(new JScrollPane(dataTable),
						BorderLayout.CENTER);
	...
	public void setupComboBoxes() {
		...
		/*
		 * **************Combo box for selecting type of data to display
		 * (order/invoice/stock)************
		 */
		...
		orderInvoiceSelectionComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				orderInvoiceSelection = orderInvoiceSelectionComboBox
		...
		productSelectionComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				selctedProductText = productSelectionComboBox.getSelectedItem()
						.toString();
			}
		});
		...
		stockRangeComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (stockRangeComboBox.getSelectedItem().toString()
						.equals("One month")) {
					chartDays = 31;
				} else if (stockRangeComboBox.getSelectedItem().toString()
						.equals("Three months")) {
					chartDays = 92;
				} else if (stockRangeComboBox.getSelectedItem().toString()
						.equals("One year")) {
	...
	public void makeChart() {
		...
		if (orderInvoiceSelection.equals("Sales")) {
			if (sb.length() != 0) { // then a product must be selected
				...
				Product product = db.getProducts().get(
		...
		JFreeChart chart = ChartFactory.createTimeSeriesChart(chartTitle,
				"Time", "€", dataFromTable(), false, false, false);
	...
	public void makeTableModel() {
		...
		// Initiate calendar (necessary for incrementing date)
		Date date = new Date();
		...
		double totalValue = 0; // cumulative sale / purchase value for certain date
			...
			for (int j = 0; j < chartDays; j = j + 7) {
				...
				for (Invoice i : db.getInvoices()) {
					...
					if (c1.get(Calendar.YEAR) == c2.get(Calendar.YEAR)
						...
						// If sb has a length (is a number) then the selection
						// must be a product
						if (sb.length() != 0) {
							// Find the product corresponding to the selected ID
							// in the item
							Product product = db.getProducts().get(
									Integer.parseInt(sb.toString()) - 1);
							// Loop through products. Only add the total sales
							// if the product is the same as the selected
							// product
	...
	public XYDataset dataFromTable() {
		...
		try {
			for (int i = 0; i < dataTableModel.getRowCount() - 1; i++) {
				...
				xValue = new Day(sdf.parse(dataTableModel.getValueAt(i, 0)
						.toString()));

				double yValue = Double.parseDouble(dataTableModel.getValueAt(i,
						1).toString());
	...
	public void productScreen() {
		...
		// Populating the products table - using vectors because they are
	...
	public void productCreateScreen() {
		...
		JLabel createOrderIDDisplayLabel = new JLabel(
				db.getNoOfProducts() + "", JLabel.TRAILING);
		...
		// row4 to hold wholesalePrice label and wholeSaleTextField &
		// retailprice label and textFields
		...
		backButton
				.setToolTipText("Cancels new product and returns to Product screen");
		...
		productCreateAddButton.addActionListener(new ActionListener() { // Create
					// product
					// from
					// fields
					public void actionPerformed(ActionEvent e) {
						if (check.isNotBlank(productCreateNameText.getText())
								&& check.isPositiveNumeric(productCreateWholesaleText
										.getText())
								&& check.isPositiveNumeric(productCreateRetailText
										.getText())) {
							db.createProduct(
									productCreateNameText.getText(),
									Double.parseDouble(productCreateWholesaleText
											.getText()),
									Double.parseDouble(productCreateRetailText
											.getText()),
									db.getSuppliers().get(
											productCreateSupplierCombo
													.getSelectedIndex()));
	...
	public void productEditScreen() {
		...
		productEditBackButton
				.setToolTipText("Cancels edit product and returns to Product screen");
		...
		productCreateAddButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (check.isNotBlank(nameText.getText())
					...
					Supplier supplier = db.getSuppliers().get(
							supplierCombo.getSelectedIndex());
	...
	public void orderScreen() {
		...
		for (Order order : db.getOrders()) {
			...
			singleOrder.add(Double.toString(order
	...
	public void orderCreateScreen() {
		...
		JLabel createOrderIDDisplayLabel = new JLabel(db.getNoOfOrders() + "",
				JLabel.TRAILING);
		createOrderIDLabel.setLabelFor(createOrderIDDisplayLabel);
		// Label & ComboBox to display order supplier
		JLabel createOrderSupplierLabel = new JLabel("Supplier:",
				JLabel.TRAILING);
		listOfSuppliersVector = new Vector<String>();
		if (orderBasket.size() > 0) {
			listOfSuppliersVector.add(orderBasket.get(0).getSupplier()
					.getName());
		} else {
		...
		createOrderSupplierCombo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (Product product : db.getProducts()) {
					if (listOfSuppliersVector.get(createOrderSupplierIndex)
							.equals(product.getSupplier().getName())) {
		...
		JLabel createOrderProductLabel = new JLabel("Product(s):",
				JLabel.TRAILING);
		listOfOrderProductsVector = new Vector<String>();
		createOrderProductCombo = new JComboBox<String>(
				listOfOrderProductsVector);
		createOrderProductLabel.setLabelFor(createOrderProductCombo);

		JLabel createOrderQuantityLabel = new JLabel("Quantity:",
				JLabel.TRAILING);
		...
		// Lay out the panel.
		SpringUtilities.makeCompactGrid(orderCreateFormPanel, 4, 2, // rows,
																	// columns
				6, 6, // initX, initY
				6, 6); // xPad, yPad
		orderBsketPanel.add(basketLabel, BorderLayout.NORTH);
		orderBsketPanel.add(new JScrollPane(orderBasketTable),
				BorderLayout.CENTER);

		JButton orderCreateBackButton = new JButton("Cancel");
		orderCreateBackButton
				.setToolTipText("Cancels new order and returns to Order screen");
		...
		orderCreateAddButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (check.isPositiveNumeric(createOrderQuantityField.getText())
						&& check.isProductSupplierMatch(createOrderProductCombo)) {
					addToBasket(
							orderBasket,
							listOfOrderProductsVector
									.get(createOrderProductCombo
											.getSelectedIndex()),
							Integer.parseInt(createOrderQuantityField.getText()));
					orderBasketRefresh();
					createOrderSupplierIndex = createOrderSupplierCombo
							.getSelectedIndex();
					listOfOrderProductsVector.clear();
					for (Product product : db.getProducts()) {
						if (listOfSuppliersVector.get(createOrderSupplierIndex)
	...
	public void orderEditScreen() {
		...
		orderEditBackButton
				.setToolTipText("Cancels editing order and returns to Order screen");
	...
	public void supplierScreen() {
		...
		// create Table for bottom panel. using DefaultTableModel as an easy way
		// to make a table
		DefaultTableModel dtm = new DefaultTableModel();
		...
		/*
		 * Populating the suppliers table - Loop through suppliers ArrayList and
		 * all each ID and name to vector Using vectors because they are
		 * compatible with tables whereas ArrayLists are not
		 */
		for (Supplier s : db.getSuppliers()) {
	...
	public void supplierCreateScreen() {
		...
		TitledBorder nameBorder = BorderFactory
				.createTitledBorder("Supplier Details");
		...
		JButton backButton = new JButton("Cancel");
		backButton
				.setToolTipText("Cancels new supplier and returns to Supplier screen");
		...
		// Set action command so that program switches to supplierScreenAccess
		// panel after successful add
		addButton.setActionCommand(supplierScreenAccess);

		// Add Supplier - when button is pressed, check if the text field is not
		// blank, then add supplier
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (check.isNotBlank(supplierCreateNameText.getText())) {
					...
					// Retrieving the current active window so that the program
					// can refresh() and then switch to the previous screen
					CardLayout cl = (CardLayout) (gui.getLayout());
					cl.show(gui, e.getActionCommand());
					currentActiveScreen = e.getActionCommand();

					// This updates the rest of the program with the new
					// supplier
					refresh();
				}
	...
	public void customerScreen() {
		...
		/*
		 * Populating the customers table - Loop through customers ArrayList and
		 * all each ID and name to vector Using vectors because they are
		 * compatible with tables whereas ArrayLists are not
		 */

		// create Table for customers panel. using DefaultTableModel as an easy
		// way to make a table
		...
		customerTable.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				selectedCustomerID = Integer.parseInt(customerTable.getValueAt(
						customerTable.getSelectedRow(), 0).toString());
			}
		});
	...
	public void customerCreateScreen() {
		...
		// ******************Date of birth panel*******************
		/*
		 * Create the Date of Birth Panel. This will contain the date labels and
		 * combo boxes and be inserted into the Spring Grid Layout
		 */
		...
		for (int i = 1; i < 32; i++) {
			// If the number is less than 10, convert to a double digit
			// (necessary for date formatting)
		...
		customerCreateBackButton
				.setToolTipText("Cancels new customer and returns to Customer screen");
		...
		/*
		 * Add customer: if all text fields contain text, create customer from
		 * text fields
		 */
		customerCreateAddButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (check.isNotBlank(customerCreateNameText.getText())
						&& check.isNotBlank(customerCreatePhoneText.getText())
						&& check.isNotBlank(customerCreateAddressText.getText())) {

					// Initiate date format so that date of birth can be passed
					// into createCustomer method
					SimpleDateFormat ft = new SimpleDateFormat("ddMMyyyy");
					Date dOB = new Date();

					// Gets the correct month as a string in two digit form
					// (01-12)
					String month = (monthsComboBox.getSelectedIndex() + 1 < 10 ? "0"
							: "")
							+ Integer.toString(monthsComboBox
									.getSelectedIndex() + 1);

					// the .parse function will not work unless surrounded by a
					// try+catch for some reason!
					try {

						// Making the date string of format ddMMyyyy
						dOB = ft.parse(daysComboBox.getSelectedItem()
								.toString()
								+ month
								+ yearsComboBox.getSelectedItem().toString());
					} catch (ParseException e1) {
					}
					// Create customer from fields
					if (check.isOfLegalAge(dOB)) {
						db.createCustomer(customerCreateNameText.getText(),
								dOB, customerCreateAddressText.getText(),
								customerCreatePhoneText.getText());
	...
	public void customerViewScreen() {
		...
		customerViewScreen.setLayout(new BoxLayout(customerViewScreen,
				BoxLayout.Y_AXIS));
		...
		editButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				for (Customer c : db.getCustomers()) {
					if (selectedCustomerID == c.getId()) {
						if (check.isNotBlank(nameText.getText())
								&& check.isNotBlank(addressText.getText())
		...
		middleInvoicesJP.add(
				new JLabel("Total money spent: "
						+ customer.getTotalMoneySpent(db.getInvoices())),
				BorderLayout.SOUTH);
	...
	public void invoiceScreen() {
		...
		/*
		 * Complete in sprint 2 JButton invoiceEditButton = new
		 * JButton("Edit Invoice");
		...
		// // Clears the ArrayList of Products that can be added to an Invoice
		// prio
		// invoiceBasket.clear();
	}
	...
	public void invoiceCreateScreen() {
		...
		JLabel createInvoiceIDDisplayLabel = new JLabel(db.getNoOfInvoices()
				+ "", JLabel.TRAILING);
		createInvoiceIDLabel.setLabelFor(createInvoiceIDDisplayLabel);
		// Label & ComboBox to display invoice customer
		JLabel createInvoiceCustomerLabel = new JLabel("Customer:",
				JLabel.TRAILING);
		...
		createInvoiceCustomerCombo = new JComboBox<String>(
				listOfCustomersVector);
		createInvoiceCustomerCombo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				invoiceCustomer = listOfCustomersVector
						.get(createInvoiceSupplierIndex);
		...
		JLabel createInvoiceProductLabel = new JLabel("Product(s):",
				JLabel.TRAILING);
		listOfInvoiceProductsVector = new Vector<String>();
		createInvoiceProductCombo = new JComboBox<String>(
				listOfInvoiceProductsVector);
		createInvoiceProductLabel.setLabelFor(createInvoiceProductCombo);

		JLabel createInvoiceQuantityLabel = new JLabel("Quantity:",
				JLabel.TRAILING);
		...
		// Lay out the panel.
		SpringUtilities.makeCompactGrid(invoiceCreateFormPanel, 4, 2, // rows,
																		// columns
				6, 6, // initX, initY
				6, 6); // xPad, yPad
		invoiceBsketPanel.add(basketLabel, BorderLayout.NORTH);
		invoiceBsketPanel.add(new JScrollPane(invoiceBasketTable),
				BorderLayout.CENTER);

		JButton invoiceCreateBackButton = new JButton("Cancel");
		invoiceCreateBackButton
				.setToolTipText("Cancels new invoice and returns to Invoice screen");
		...
		invoiceCreateAddButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				if (check.isPositiveNumeric(createInvoiceQuantityField
						.getText())
						&& check.isInvoiceProductSelected(createInvoiceProductCombo)
						&& check.isSufficientStock(db, invoiceBasket,
								listOfInvoiceProductsVector
										.get(createInvoiceProductCombo
												.getSelectedIndex()), Integer
										.parseInt(createInvoiceQuantityField
												.getText()))) {
					addToBasket(invoiceBasket, listOfInvoiceProductsVector
							.get(createInvoiceProductCombo.getSelectedIndex()),
							Integer.parseInt(createInvoiceQuantityField
									.getText()));
					invoiceBasketRefresh();
					createInvoiceSupplierIndex = createInvoiceCustomerCombo
							.getSelectedIndex();
					invoiceCustomer = listOfCustomersVector
							.get(createInvoiceSupplierIndex);
					listOfInvoiceProductsVector.clear();
	...
	private class OrderSupplierListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			listOfOrderProductsVector.clear();
			for (Product product : db.getProducts()) {
				if (db.getSuppliers().get(createOrderSupplierIndex) == product
						.getSupplier()) {
					listOfOrderProductsVector.add(product.getName());
	...
	public void orderSubmit(ArrayList<Product> prodArray) {
		ArrayList<Product> basketContents = new ArrayList<Product>(
				(ArrayList<Product>) prodArray.clone());
