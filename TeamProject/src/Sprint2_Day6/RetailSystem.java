...
public class RetailSystem { 
    ...
    public void createOrder(ArrayList<Product> products){ 
        ...
        int deliveryDays = rg.nextInt(2)+1;
        ...
        deliveryDate = calendar.getTime(); //Set the delivery date to the random future date
    ...
    public void recordDeliveries(){
    	...
    	for (Order order : orders){
    		...
    		if (order.getDeliveryDate().getYear()==NewUI.systemDate.getYear()  				 				
    				&& order.getDeliveryDate().getMonth()==NewUI.systemDate.getMonth()
    				&& order.getDeliveryDate().getDay()==NewUI.systemDate.getDay()){
